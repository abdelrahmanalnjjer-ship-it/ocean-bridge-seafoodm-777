...
function HeroVideos() {
  const videos = [vlog1, vlog2];
  const [i, setI] = useState(0);
  useEffect(() => {
    const id = setInterval(() => setI((v) => (v + 1) % videos.length), 8000);
      ...
      <div className="absolute bottom-8 right-8 z-10 flex gap-2">
        {videos.map((_, idx) => (
          <button
            key={idx}
            onClick={() => setI(idx)}
            aria-label={`Hero video ${idx + 1}`}
            className={`h-[2px] transition-all ${idx === i ? "w-10 bg-[#C73E1D]" : "w-6 bg-white/40 hover:bg-white/70"}`}
          />
        ))}
      </div>
    </div>
